...
Questo caso d'uso descrive il flusso di una transazione che viene interrotta dopo il blocco temporaneo del buono. Questo scenario si verifica quando il cittadino decide di non procedere più con l'acquisto.
...
L'Operatore PV ha precedentemente accettato un buono sconto (come da tutorial [2.4 Accettare un buono sconto](https://www.google.com/search?q=tutorial/accettare-buono-sconto)). Esiste quindi una transazione nello stato "temporaneamente bloccato" nella sezione "Gestione Acquisti".
...
* **Buono da App IO**: Il buono torna disponibile sull'app del cittadino entro **5 minuti**.
