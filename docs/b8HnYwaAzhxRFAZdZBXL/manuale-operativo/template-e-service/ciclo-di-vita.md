...
Per maggiori dettagli sulla struttura dell'e-service, si veda la [sezione dedicata](../e-service.md#creare-un-nuovo-e-service).
...
Un ente che ha creato un template può anche utilizzarne un'istanza per se stesso. Il vincolo rimane che potrà istanziare un solo e-service da un proprio template, per evitare che l'erogatore abbia più e-service con lo stesso nome e scopo. Anche in questo caso, fanno eccezione gli e-service ricevuti in gestione per delega, come descritto nella [sezione dedicata](../deleghe.md).
