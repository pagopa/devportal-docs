...
* il relativo _Referrer_, ovvero una stringa testuale di massimo 20 caratteri, anch'essa predeterminata dall’Operatore e assegnata alla singola opportunità.
...
Il parametro _Referrer_ deve essere controllato ed accettato dai sistemi dell’Operatore per tutta la durata delle opportunità erogate ai Beneficiari.&#x20;
...
* la landing page configurata dall’Operatore per la fruizione dell’opportunità viene aperta in webview valorizzando il custom header HTTP **`X-PagoPa-CGN-Referrer`**.

Il controllo del relativo _header_ può essere effettuato sia a livello infrastrutturale (Apache/Nginx) sia applicativo di back-end della pagina web dell’Operatore. Di seguito vengono riportati alcuni esempi implementativi per il check del _Referrer_ in diversi linguaggi:&#x20;
