...
I dati inseriti non hanno una scadenza. Pertanto, una volta fatta la prima configurazione è possibile far riferimento ad essa per tutte le successive invocazioni. L'eventuale rimozione dei dati può essere fatta utilizzato le API previste dalla Data Preparation.
...
Si tratta, dunque, di un metodo di verifica a uso del fruitore. Può essere invocato a seconda delle esigenze per analisi interne sulla configurazione effettuata sino a quel momento.
...
Per ogni e-service è prevista una chiave univoca che identifica l'entità (può ad esempio essere l'id del soggetto nel caso di e-service “check Id Subject)
