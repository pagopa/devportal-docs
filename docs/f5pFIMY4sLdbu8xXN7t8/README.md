...
* **Atto**: il documento informatico oggetto della notifica depositato su SEND dalla PA mittente, inteso come atto, provvedimento, avviso o comunicazione, reso disponibile telematicamente dall’amministrazione mittente su SEND come documento informatico crittografato;
...
* **Indirizzo fisico (del destinatario)**: l’indirizzo al quale viene inoltrata una comunicazione cartacea;
...
* **Raccomandata Semplice**: raccomandata informativa prevista nel caso in cui non sia possibile consegnare digitalmente l'AAR per PEC satura/ non valida/ non attiva;
...
Servizio Notifiche Digitali (SEND), anche nota come Piattaforma Notifiche Digitali di cui all'art. 26 del decreto-legge 76/2020 s.m.i. (di seguito “SEND”),  è la piattaforma di gestione della notificazione di atti amministrativi che gestisce il ciclo di vita completo della notifica, dalla sua creazione da parte della Pubblica Amministrazione (PA) Mittente, fino all’esecuzione dell’eventuale pagamento dell'atto oggetto di notifica da parte del destinatario.

SEND semplifica la gestione del processo  di notificazione, la Pubblica Amministrazione mittente deve occuparsi del solo deposito su SEND dell’atto da notificare;  sarà poi la piattaforma a occuparsi dell’invio, per via digitale o analogica.
...
3. **FAQ Enti**: le FAQ sono disponibili a [questo riferimento](https://docs.pagopa.it/faq-enti).
...
5. **Lista codici tassonomici**: La lista dei codici tassonomici è reperibile tramite l’[apposita pagina](https://docs.pagopa.it/f.a.q.-per-integratori/tassonomia-send) della documentazione tecnica.
6. **Link Privacy**: la PA mittente deve inviare al team Privacy di PagoPA S.p.A. il link della propria informativa sulla privacy via mail all’indirizzo pn-linkinformativa@pagopa.it.
...
8. [**Manuale operativo**](https://docs.pagopa.it/manuale-operativo-pn/v/v1.0): il Manuale operativo di SEND è disponibile sul [Sito vetrina](https://docs.pagopa.it/manuale-operativo/).
9. **Descrizione**[ **API (B2B; invio notifica)**](https://petstore.swagger.io/?url=https://raw.githubusercontent.com/pagopa/pn-delivery/develop/docs/openapi/api-external-b2b-pa-bundle.yaml): la descrizione delle API utilizzate dalle Pubbliche Amministrazioni per inviare richieste di notifiche e ottenere informazioni sullo stato della richiesta di notifica e sulle comunicazioni effettuate, sono disponibili sul [Sito vetrina.](https://petstore.swagger.io/?url=https://raw.githubusercontent.com/pagopa/pn-delivery/pn-openapi-prod/docs/openapi/api-external-b2b-pa-bundle.yaml)
10. [**Knowledge Base di SEND**](https://docs.pagopa.it/f.a.q.-per-integratori/): il documento che raccoglie gli insight tecnici di SEND è disponibile sul [Developer Portal](https://developer.pagopa.it/).
11. [**Modello di integrazione**](https://docs.pagopa.it/modello-di-integrazione-di-piattaforma-notifiche/): il documento che descrive i diversi scenari di integrazione di SEND è disponibile sul [Developer Portal](https://developer.pagopa.it/).
12. [**Test di validazione Validator Tool**](https://docs.pagopa.it/f.a.q.-per-integratori/knowledge-base-di-piattaforma-notifiche/pn-test-di-validazione-avvenuta-integrazione-con-piattaforma-notifiche): il documento che raccoglie i test che una PA mittente, o il suo partner tecnologico, devono effettuare e documentare per validare l’avvenuta integrazione con SEND è disponibile sul [Developer Portal](https://developer.pagopa.it/).

La documentazione elencata nei punti da 7 a 12 è inoltre disponibile sul [Developer Portal](https://developer.pagopa.it/).
...
* **Adempimenti amministrativi e preliminari (Owner: Ente)**: messa a disposizione e sottoscrizione dell’accordo di adesione dell’Ente e ulteriore documentazione necessaria, attestazione PT, fatturazione.
...
## Adempimenti amministrativi e preliminari

Gli adempimenti amministrativi (unitamente a quelli di carattere non amministrativo) e le relative scadenze sono da rispettare durante il corso di tutto il processo.&#x20;
...
Verificato che il soggetto sia in perimetro SEND e individuato il Partner Tecnologico (PT), il soggetto procederà alla sottoscrizione dell’accordo di adesione secondo le seguenti modalità:&#x20;
...
2. Ricevere, compilare e inviare all’indirizzo PEC pagopa@pec.governo.it il Contratto di Adesione firmato digitalmente e il Documento di Termini e Condizioni compilato ove richiesto;
3. Inviare via mail ([pn-linkinformativa@pagopa.it](mailto:pn-linkinformativa@pagopa.it)) il link dell’informativa sul trattamento dei dati pubblicata sul proprio sito a PagoPA S.p.A.

![Immagine che contiene testo, schermata, cerchio, Carattere

Descrizione generata automaticamente](https://lh7-eu.googleusercontent.com/OYqmaiAuapVuzJSx76kr0tZ9OozT2oWcTD1vI-KKcY3L6Ppwhm2vkBWgu2ft1TaaQoiRDsR-x47gqjzJdFh81JslEUfDJzPLbc20TxrE77l51srWXiYQLRsO9Elnvt4TFLMJ2i5_gPwMayXADFpjrw)



### Generazione API key

Tramite l’[Area Riservata](https://www.pagopa.it/it/area-riservata/), l’Ente deve generare le API key di produzione e accedere alla documentazione di SEND che guida le PA e i propri Partner Tecnologici (PT) nel processo di integrazione. Una volta eseguito l’accesso a [Area Riservata](https://selfcare.pagopa.it/), cliccando sul prodotto attivo “SEND Servizio Notifiche Digitali” il soggetto ha modo di generare le API key che deve consegnare al Partner Tecnologico (PT) scelto, unitamente alla documentazione tecnica a supporto dell’integrazione.&#x20;
...
### Compilazione e invio modulo di profilazione.

Entro 30 giorni dalla firma del contratto di adesione, l’Ente deve scaricare il Modulo di profilazione dal [Sito ](https://notifichedigitali.pagopa.it/pubbliche-amministrazioni/)di SEND e inviarlo a mezzo PEC (pagopa@pec.governo.it). All’interno del modulo è necessario indicare il numero di notifiche complessive che si prevede di inviare tramite processo nel corso dell’anno, con specifica indicazione del numero di notifiche da processare in via digitale e in via analogica (distinguendo tra atti da notificare tramite raccomandata A/R e atti da notificare tramite raccomandata 890) per ciascun mese dell’anno solare di riferimento. La compilazione di tale pagina sarà richiesta nuovamente ogni anno, entro il 31 ottobre.

### PDND Interoperabilità – soggetti in perimetro PDND Interoperabilità

Per i soggetti rientranti nell’ art. 2 comma 2 CAD è possibile inviare notifiche selezionando l’e-service SEND attraverso PDND Interoperabilità. A tal fine tali Enti sono tenuti ad accreditarsi anche a PDND Interoperabilità nelle modalità indicate nella [Guida all’adesione](https://docs.pagopa.it/interoperabilita-1/manuale-operativo/guida-alladesione).
...
1. ### Perimetro complessivo di use-case
...
2. ### Prioritizzazione e Valutazione di Impatto&#x20;
...
È essenziale che in questa fase l’Ente valuti di quale tipologia di integrazione usufruire, consultando il  [modello di integrazione](https://docs.pagopa.it/modello-di-integrazione-di-piattaforma-notifiche/) di SEND:&#x20;
...
3. completare gli sviluppi, con il supporto del PT, utilizzando come guida quanto indicato nel Developer Portal.
...
Dopo aver generato le API Key, gli step da seguire per sviluppare l’integrazione con SEND sono i seguenti:

1. Generazione dei client a partire dalla documentazione Swagger per la pagina API B2B per le Pubbliche Amministrazioni;
2. (Se prevista) [integrazione a SEND tramite PDND Interoperabilità](https://docs.pagopa.it/interoperabilita-1/manuale-operativo/guida-alladesione):
...
3. Creazione e gestione di uno o più stream per recupero esiti.
4. Inserimento della notifica: caricamento documenti e inserimento notifica.&#x20;
5. Dove richiesto,[ attualizzazione della posizione debitoria](https://docs.pagopa.it/modello-di-integrazione-di-piattaforma-notifiche/).[ ](https://docs.pagopa.it/pnvalidator/)
6. [Test di validazione](https://docs.pagopa.it/f.a.q.-per-integratori/knowledge-base-di-piattaforma-notifiche/pn-test-di-validazione-avvenuta-integrazione-con-piattaforma-notifiche) e avvenuta integrazione con SEND.&#x20;
7. Ottimento report generato tramite il  Validator Tool e invio a PagoPA  S.p.A. del buon esito tramite [ticket Jira](https://pagopa.atlassian.net/servicedesk/customer/portal/5/group/28/create/150).
8. PagoPA S.p.A. invierà conferma dell'avvenuta integrazione in UAT.

Per dettagli sugli step sopra riportati, fare riferimento alla pagina GitBook [F.A.Q. Integratori](https://docs.pagopa.it/f.a.q.-per-integratori/) e sezione Quick Start [Developer Portal](https://developer.pagopa.it/).

Con particolare riferimento ai test di validazione, PagoPA S.p.A. mette a disposizione un tool che permette ai Partner Tecnologici degli Enti di eseguire una simulazione minimale dei servizi erogati da SEND e un endpoint che produce un report sull’avvenuta integrazione ([Validator tool](https://docs.pagopa.it/f.a.q.-per-integratori/knowledge-base-di-piattaforma-notifiche/focus-sulla-validazione-dei-test-con-il-tool-pnvalidator)).
...
Ogni mese dal momento di avvio in esercizio, SEND fatturerà all’Entemittente le spese effettive di notificazione sostenute per ciascun destinatario della notifica.
...
| <mark style="color:blue;">**Ambito**</mark> | <mark style="color:blue;">**Riferimenti utili**</mark>   | <mark style="color:blue;">**Quando contattare**</mark>                           | <mark style="color:blue;">**Indicazioni ulteriori**</mark>                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
