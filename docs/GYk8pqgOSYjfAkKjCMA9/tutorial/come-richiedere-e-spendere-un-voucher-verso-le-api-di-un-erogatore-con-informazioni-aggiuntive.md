# Come richiedere e spendere un voucher verso le API di un erogatore (con informazioni aggiuntive)

Il flusso di autenticazione implementato è un OAuth 2.0 che fa riferimento all'[RFC 6750](https://datatracker.ietf.org/doc/html/rfc6750) per quanto riguarda l'uso del Bearer token e all'[RFC 7521](https://datatracker.ietf.org/doc/html/rfc7521) per l'autorizzazione del client tramite client assertion. Inoltre, il JWS contenente le informazioni aggiuntive rispetta l'[RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519).&#x20;

Il pattern individuato per l'inoltro di informazioni aggiuntive è quello previsto da AgID nel MoDI, indicato come Audit REST 02. Per maggiori informazioni, si veda la [sezione dedicata](../guida-tecnica/utilizzare-i-voucher/ciclo-di-vita.md#bearer-token-spendibile-presso-le-api-di-un-erogatore-con-informazioni-aggiuntive-pattern-modi-audit).
...
* generato almeno un set di materiale crittografico e caricato la relativa chiave pubblica su PDND Interoperabilità all'interno del client ([vedi tutorial](come-generare-corredo-crittografico-e-caricare-chiavi-pubbliche.md));
...
## Step 4 - Richiesta del voucher al server autorizzativo di PDND Interoperabilità
...
## Step 5 - Attendere le verifiche del server di PDND, che rilascia il voucher
