...
Se viene pubblicato passa in stato **Attivo** ed è utilizzabile dai fruitori.

Dopo essere stato pubblicato, un e-service per effettuare modifiche bisogna **creare una nuova versione**. Quando una nuova versione viene pubblicata, la precedente diventa **deprecata**, rimanendo disponibile fino a quando tutti i fruitori non avranno aggiornato le loro richieste di fruizione.
...
Un e-service può essere in **stato deprecato** qualora il servizio sia stato aggiornato con una nuova versione, ma rimane disponibile per garantire continuità.
